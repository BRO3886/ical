...
## Session 3 - Rename `cal` → `ical`

## Goal
Rename the binary and all references from `cal` to `ical` to avoid conflicting with the POSIX `cal` utility (displays a calendar). Full rename: binary, module path, directory, imports, user-facing strings, docs, and tagged release.

## What Changed

### Go source (16 files)
- `git mv cmd/cal cmd/ical` — directory rename (changes `go install` binary name)
- `go.mod`: `module github.com/BRO3886/cal` → `module github.com/BRO3886/ical`
- All 11 Go files with imports: `github.com/BRO3886/cal/` → `github.com/BRO3886/ical/`
- `main.go:8`: import path also needed `cmd/cal/` → `cmd/ical/` (the sed only replaced the module prefix `cal/`, not the nested `cmd/cal/` — had to fix manually)
- `root.go`: `Use: "cal"` → `Use: "ical"`, Long description updated
- `version.go`: `"cal %s"` → `"ical %s"`
- `main.go`: `"cal requires macOS"` → `"ical requires macOS"`
- `output.go`: `.cal-last-list` → `.ical-last-list`
- `ics.go`: `PRODID:-//cal CLI//EN` → `PRODID:-//ical CLI//EN`

### Build files
- `Makefile`: `bin/cal` → `bin/ical`, `./cmd/cal` → `./cmd/ical`, completion filenames (`cal.bash` → `ical.bash`, `_cal` → `_ical`, `cal.fish` → `ical.fish`)
- `.gitignore`: `/cal` → `/ical` (kept `/cal` too for transition)

### Documentation (12 files)
- `README.md`: title, install commands, all `cal ` command examples, repo URL, git clone, architecture tree, shell completion paths
- `CLAUDE.md`: title, repo URL, build commands, cache path, architecture tree
- `website/config.yaml`: title, GitHub repo URLs, keywords
- `website/content/docs/` (4 files): all command examples, cache path refs, install commands
- `website/themes/cal-docs/layouts/index.html`: install commands, git clone URL
- `skills/cal-cli/SKILL.md` + `references/commands.md` + `references/dates.md`: all command examples
- `docs/prd/cal-cli-prd.md`: module path, all command refs

### Unchanged (intentional)
- Hugo theme directory name (`cal-docs`) — just a theme name, not user-facing
- Cloudflare Pages project name (`cal`) and domain (`cal.sidv.dev`)
- Skills directory name (`cal-cli`) — directory name, not a command
- Journals — historical records
- `website/public/` — generated Hugo output (gitignored), will regenerate on next Hugo build

### Release
- Tagged `v0.3.0`, pushed, created GitHub release with macOS binary and migration notes

## Key Insights
- **sed `s/cal/ical/g` is dangerous** for this codebase: words like "Local", "calendar", "cal-docs", "cal-cli", "cal.sidv.dev" all contain "cal". Had to use very targeted patterns with word boundaries and context.
- **Import path double-rename gotcha**: `github.com/BRO3886/cal/cmd/cal/commands` has TWO `cal/` segments. A single `s/cal\//ical\//` only replaces the first occurrence, leaving `github.com/BRO3886/ical/cmd/cal/commands`. Need to replace the full path or use `g` flag carefully.
- **`website/public/` stale output**: Hugo build output still has old references but is gitignored. Will auto-fix on next Hugo build/deploy.
- **Go module rename is seamless**: Since this is a private repo with no external consumers, `go.mod` module path change works without any `replace` directives or vanity import paths.

## Decisions Made
- **v0.3.0 tag**: Semver minor bump since this is a breaking change (binary name) but no API changes
- **Keep `/cal` in .gitignore**: Added `/ical` but kept `/cal` for transition (user may have old binary lying around)
- **Don't rename `cal-docs` theme**: It's an internal theme directory name, not shown to users. Renaming would require updating Hugo config and template references for no user benefit.
- **Don't rename `cal-cli` skills dir**: The directory name is metadata, not a command name. Renaming would break any existing agent integrations.
- **Don't rename Cloudflare project**: Would require new DNS setup. Domain `cal.sidv.dev` is fine — it's a URL, not a command.
